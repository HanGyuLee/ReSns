...
public class YdhUtil {
	...
	}// end of String getPageBarWithSearch(int sizePerPage, int blockSize, int totalPage, int currentShowPageNo, String colname, String search, String period, String url)--------------------	
	...
	public static String getCurrentURL(HttpServletRequest req) {
		...
		if("GET".equalsIgnoreCase(method)){
			...
			currentURL += "?" + queryString; // ?userid=~~&name=~~&content=~~
		}// end of if(get)
...
/*	public static String getPageBar(String search,String colname,int blockSize, int sizePerPage, int currentShowPageNo,int totalPage, String url ){
		...
		int pageNo =((currentShowPageNo -1)/blockSize)*blockSize +1;//공식
			...
			currentShowPageNo : 1  ~ 10 일 떄 pageNo 는 1
			currentShowPageNo : 11 ~ 20 일 떄 pageNo 는 11
			currentShowPageNo : 21 ~ 30 일 떄 pageNo 는 21	
		...
		//	while(pageNo<totalPage){
			...
			if(pageNo == 1)
				...
				str_pageNo = "&nbsp;<a href='"+url+"?colname="+colname+"&search="+search+"&sizePerPage="+sizePerPage+"&currentShowPageNo="+(pageNo-10)+"'>[ 이전"+ blockSize +"페이지 ]</a>&nbsp;";
			
			pageBar += str_pageNo;
			
		
			while(!(pageNo>totalPage || loop > blockSize)){	
				if(currentShowPageNo == pageNo)
				str_pageNo = "&nbsp;<span style='font-size:13pt; text-decoration:underline; font-weight:bold;'>"+pageNo+"</span>&nbsp;";
				else
				str_pageNo = "&nbsp;<a href='"+url+"?colname="+colname+"&search="+search+"&sizePerPage="+sizePerPage+"&currentShowPageNo="+pageNo+"'>"+ pageNo +"</a>&nbsp;";
			...
			if(pageNo>totalPage)
				...
				str_pageNo = "&nbsp;<a href='"+url+"?colname="+colname+"&search="+search+"&sizePerPage="+sizePerPage+"&currentShowPageNo="+pageNo+"'>[ 다음"+ blockSize +"페이지 ]</a>&nbsp;";
